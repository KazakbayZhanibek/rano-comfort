    ...
    <main style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: '0.9rem' }}>Войдите в свой аккаунт</p>
        </div>
        ...
        <div style={{ padding: '2rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.375rem' }}>
                ...
                <Link href="/auth/forgot" style={{ fontSize: '0.8rem', color: 'var(--color-secondary)', fontWeight: 500 }}>
                  Забыли пароль?
                </Link>
              </div>
              <div style={{ position: 'relative' }}>
                ...
                <input className="input" type={showPass ? 'text' : 'password'} placeholder="Введите пароль"
                  value={form.password} onChange={e => setForm({ ...form, password: e.target.value })}
                  style={{ paddingLeft: '2.75rem', paddingRight: '2.75rem' }} />
                <button type="button" onClick={() => setShowPass(!showPass)} style={{ position: 'absolute', right: '0.875rem', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)', padding: 0, display: 'flex' }}>
          ...
          <p style={{ textAlign: 'center', fontSize: '0.9rem', color: 'var(--color-text-muted)' }}>
            Нет аккаунта?{' '}
            <Link href="/auth/register" style={{ color: 'var(--color-primary)', fontWeight: 700 }}>
              Зарегистрироваться
            </Link>
          </p>
        </div>
